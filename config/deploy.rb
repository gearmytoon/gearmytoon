...
namespace :vlad do
  ...
  remote_task :import_a_items_text_file do
    run "cd #{current_path} && rake import_a_items_text_file NAME=#{ENV['NAME']}"
  end

  desc "import all text files via wow armory on production"
  remote_task :import_all_items_from_text_files do
    run "cd #{current_path} && rake import_all_item_text_files"
  end

  desc "import all items that match a term from wow armory on production"
  remote_task :import_pvp_gear do
    run "cd #{current_path} && rake import_by_wow_armory_search TERM=\"Relentless Gladiator's\""
    run "cd #{current_path} && rake import_by_wow_armory_search TERM=\"furious Gladiator's\""
    run "cd #{current_path} && rake import_by_wow_armory_search TERM=\"merciless Gladiator's\""
  end
  ...
  namespace :jobs do
    ...
    remote_task :start do
      run "cd #{current_path} && rake jobs:start"
    end

    desc "stop delayed_job worker"
    remote_task :stop do
      run "cd #{current_path} && rake jobs:stop"
    end
